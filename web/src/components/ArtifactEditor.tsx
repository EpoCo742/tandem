...
export function ArtifactEditor({ artifact: a, sessionId, onClose }: { artifact: Artifact; sessionId: string; onClose: () => void }) {
  const [text, setText] = useState(a.type === "contract" ? (a.current.content as { body: string }).body : a.type === "use_case" ? JSON.stringify(Object.fromEntries(Object.entries(a.current.content as Record<string, unknown>).filter(([k]) => k !== "sections")), null, 2) : contentText(a.type, a.current.content));
  ...
  async function save() {
    ...
    else if (a.type === "data_model" || a.type === "arch_model" || a.type === "constraints" || a.type === "alternatives" || a.type === "use_case") {
      try {
