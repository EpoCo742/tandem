...
const GROUPS: Group[] = [
  {
    ...
    entries: [
      ...
      { name: "Export a session file", where: "The … menu next to a session's title (top bar or home page); or Home, Your sessions, Select… / Export all", how: "One JSON file with the whole session: conversation, cards and every version, decisions, uploads, canvas layout, published pages. Select… ticks several sessions for one file; Export all takes every session you are in. Credentials never go in the file." },
      { name: "Import session files", where: "Home page, Your sessions, Import…", how: "Pick a file exported here or on another install. Add as new sessions makes copies you own (safe; nothing here is touched). Restore in place keeps every id and published page and replaces a same-id session you own; use it to put a backup back." },
