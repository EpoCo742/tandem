...
**Still to do.** Running stdio servers in a per-user sandbox rather than as children of the archloom server; validating the Copilot runtime's MCP path on a real seat (the offline provider path is covered by the smoke test). Shipped later on 2026-09-05: standing permissions ("Approve, always for space ARCH", listed and revocable under External tools) and external actions in the History tab and the export.
...
- *Sandboxing.* Stdio servers run as child processes of the archloom server; production would run them in a per-user sandbox and the design already reserves that for M4.
