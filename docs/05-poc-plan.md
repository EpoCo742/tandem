...
2026-09-05, roadmap item A5: as-is from code. `shared/src/scan.ts` turns manifests (package.json and workspaces, docker-compose, go.mod, requirements.txt, pom.xml, Cargo.toml) into components and relationships; `manifestPaths` picks what to read from a tree. `set_as_is` records an `asIs` snapshot on the architecture model (and makes it the model when the model is empty); `modelDiff` and the `diff` view kind draw to-be against as-is. The demo MCP server gained read-only `repo_tree` and `repo_file` tools; the offline provider reads a named repository through the speaker's tool (reads need no approval) or an attached manifest; the system prompt tells a real model to read manifests only.

2026-09-05, roadmap item B3: review and sign-off. `Role` gains `reviewer`; invites carry a role; governance turns a reviewer's own changes into proposals. Events `review.requested`, `review.signed`, `review.approved`, `review.withdrawn`; `state.reviews` (status draft / in_review / approved, reviewers, signoffs, history, changedSince) is derived in the reducer, which also moves an approved document back to draft on any `artifact.applied` and drops signatures when the document itself changes during review. Routes under `/review/:artifactId/{request,sign,withdraw}`; the last signature records a decision agreed by the signers. Digest lists pending sign-offs; export shows a Status line.

2026-09-05, roadmap item A4: alternatives. `propose_alternatives` creates an `alternatives` card (question, two or three candidates each with a complete `ArchModelContent`, pros, cons, constraints met and at risk). `POST /alternatives/:id/decide` opens a decision point whose options are the candidates and which blocks the model and views; the vote route applies the outcome itself (`adoptAlternative`: model set from the winner with provenance carried over, card marked chosen, decision recorded with every candidate as an option, `alternative.adopted` system message, commit) with no AI turn. Change requests gained `force` for outcomes people already voted on.

2026-09-05, constraints belong to their setter: amending, excepting (`exceptionTo` on `upsert_constraints`) or removing a constraint someone else set is proposed to that person through a new `approvalFrom` on change requests, regardless of who owns the card; document-sourced constraints fall back to the card's owner.

2026-09-05, roadmap item B2: threads anchored to cards. A side-channel note can carry an anchor (`{ artifactId, componentId? }`); replies point at the first message and inherit the anchor; `thread.resolved` closes or reopens a thread. Promotion copies the anchor onto the promoted message, the prompt tags it `[about: card › component (artifact, component)]` and quotes the rest of the thread as background, and the system prompt tells the model that "this" means that thing. The web app has a thread panel on every card (and per component on the model table), anchored notes in the side channel, an "about" chip on promoted messages, and the export a Discussion threads section.
