# archloom: Architecture Document
...
7. **Bring your own credentials, keep your own bill.** archloom never resells inference. Each participant pays their provider directly.
...
sequenceDiagram
    ...
    participant API as archloom API
    participant KMS
...
**What archloom never does.** It never offers Claude.ai login, never stores Claude.ai session tokens, never proxies a subscription for someone who did not authenticate it. See `01-research-findings.md` section 2.
