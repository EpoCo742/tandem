...
The product is **archloom**: the session before the build, where the architecture is agreed. These documents were written under the earlier working name Tandem, which the code, package names and environment variables still use.
...
| Provider | Auth in archloom | Bills to | Claude models available | Policy status |
|---|---|---|---|---|
...
- **MUCA: Multi-User Chat Assistant** (arXiv 2401.04883). Frames the group-chat assistant problem as three decisions: *what* to say, *when* to speak, *who* to address. Three modules: sub-topic generator, dialog analyzer, conversational strategies arbitrator. archloom adopts the 3W framing: users explicitly address the AI in v1, and an "ambient" mode later lets the AI decide when to intervene.
- **GroupGPT** (arXiv 2603.01059). Separates the *intervention decision* from *response generation*, cutting token use up to 3x. archloom uses the same split: a cheap classifier step (Haiku 4.5) decides whether a message needs a full turn, a conflict flag, or nothing.
...
- **Multi-party turn-taking and addressee prediction** (several 2024-2026 papers). LLMs do reasonably at addressee recognition when speakers are explicitly labelled. archloom always labels speakers in the transcript.
