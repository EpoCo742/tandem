...
**Why.** It makes archloom the place decisions are made, not merely discussed, and leaves the repository with the artefact architects already keep.
...
- **Sandboxed MCP servers.** Stdio servers run as children of the archloom server; production needs a per-user sandbox. Production item; unchanged.
...
| F8 Version comparison | Any two versions of the design document compared by computation (text by section, decisions, model, constraints, contracts, assumptions, questions, cards) with a ranked major-changes list; saved as a card without an AI turn; an optional AI turn writes "What matters" on top; "changes since v n" from the publish panel; a fork compares with the original session's versions. A guide page in the top bar lists every feature, where it is and how to use it. | shipped |
| F13 Use cases | One Use cases card holding actors, use cases, who does what, include and extend, and the components that realise each use case; the card draws its own diagram; actors link to model components; the impact panel and the remove tool list the use cases a component touches; export and design document get a table; `upsert_use_cases` tool; PlantUML use case diagrams import into the same card. Not in the library index yet. | shipped |
| F12 What the AI is doing | While a turn runs the lane shows a moving indicator and, in plain words, the current step and the finished ones: thinking, each tool call labelled from its input (drawing, updating, recording), writing the reply; Copilot runtime start and calls to the person's own tool servers included. Derived from the calls, no tokens. | shipped |
| F11 Sessions as files | Export one, several or all sessions as a bundle (ledger, people, uploads, layout, published pages; no credentials); import as new copies or restore in place; operator routes and a script for whole-install backup and restore around a redeploy, plus a consistent SQLite snapshot. | shipped |
| F10 Zoom in full screen | Full-screen cards and presentation slides zoom with buttons, + − 0 f keys and Ctrl+wheel; fit sizes a diagram to the space. | shipped |
| F9 Undo and redo | Ctrl+Z / Ctrl+Y for the person's own last card move, resize, tidy, edit or delete in this tab; layout through a Yjs undo manager, content by writing the previous state back as a new version; a restore route for deleted cards. | shipped |
