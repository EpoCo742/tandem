  ...
  async runTurn(req: TurnRequest): Promise<TurnResult> {
    ...
    if (outbound) {
      ...
      const args = wantsTicket
        ? { project: outbound.text.match(/\b([A-Z]{2,6})\b(?!.*\b[A-Z]{2,6}\b)/)?.[1] ?? "ORD", summary: `Implement ${doc?.title ?? "the design"}`, description: `From archloom session ${req.sessionId}` }
        : { space: outbound.text.match(/\b(?:space|under|to)\s+([A-Z]{2,8})\b/)?.[1] ?? "ARCH", title: doc?.title ?? "Design document", body: `Exported from archloom session ${req.sessionId} (${doc?.id ?? "no document yet"}).` };
