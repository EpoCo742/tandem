...
export async function registerLibraryRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { slug: string }; Querystring: { v?: string } }>("/p/:slug.md", async (req, reply) => {
    ...
    const a = doc.version.approval;
    const head = [
      `<!-- published from archloom: ${doc.sessionTitle}; version ${doc.version.no} (document v${doc.version.docVersionNo}), ${doc.version.publishedAt}, by ${doc.version.publishedBy}; ${a ? `approved (${a.decisionLabel}), signed off by ${a.signerNames.join(", ")}` : "not signed off"} -->`,
      "",
    ];
