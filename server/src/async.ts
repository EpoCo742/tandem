...
export function digestFor(userId: string): DigestSession[] {
  ...
  for (const row of rows) {
    const meta = db.select({ status: schema.sessions.status, demo: schema.sessions.demo }).from(schema.sessions).where(eq(schema.sessions.id, row.sessionId)).get();
    if (!meta || meta.status === "archived" || meta.demo > 0) continue; // nothing can be waiting in a read-only session
